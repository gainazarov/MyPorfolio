        ...
        <div className="grid lg:grid-cols-2 gap-8 mb-16 items-stretch">
          {projects.map((project, index) => {
            const card = (
              <motion.div 
                key={project.id}
                className="brutalist-card p-8 group hover:scale-105 transition-transform duration-300 cursor-pointer h-full min-h-[600px] flex flex-col"
                initial={{ opacity: 0, y: 30 }}
                animate={isVisible ? { opacity: 1, y: 0 } : {}}
            ...
            return project.link ? (
              <a
                ...
                className="block focus:outline-none focus:ring-2 focus:ring-accent h-full"
              >
                {card}
              </a>
            ) : card;
